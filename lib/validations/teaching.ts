...
export const teachingSchema = z
  .object({
    ...
    description: z.string().trim().min(10, "La description doit contenir au moins 10 caractères.").max(2000),
    youtubeUrl: urlOrEmpty.refine(
      (value) => !value || extractYoutubeId(value) !== null,
      "Lien YouTube non reconnu. Utilisez le lien de la vidéo (ex : https://www.youtube.com/watch?v=...).",
    ),
...
export function getTeachingWarnings(data: { status: string; coverImageUrl?: string | null }): string[] {
  ...
  if (!data.coverImageUrl) {
    warnings.push("Aucune image de couverture : une image par défaut sera affichée.");
  }
  return warnings;
}
